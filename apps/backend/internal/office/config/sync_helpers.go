...
func (s *ConfigService) appendDeletions(
...
func (s *ConfigService) deleteRowsMissingFromBundle(
...
func (s *ConfigService) deleteMissingAgents(
	ctx context.Context, workspaceID string, bundle *ConfigBundle,
) error {
	rows, err := s.repo.ListAgentInstances(ctx, workspaceID)
	if err != nil {
		return err
	}
	...
	for _, a := range rows {
		...
		if err := s.repo.DeleteAgentInstance(ctx, a.ID); err != nil {
			return err
		}
	}
	return nil
...
func (s *ConfigService) deleteMissingSkills(
	ctx context.Context, workspaceID string, bundle *ConfigBundle,
) error {
	rows, err := s.repo.ListSkills(ctx, workspaceID)
	if err != nil {
		return err
	}
...
func (s *ConfigService) deleteMissingRoutines(
	ctx context.Context, workspaceID string, bundle *ConfigBundle,
) error {
	rows, err := s.repo.ListRoutines(ctx, workspaceID)
	if err != nil {
		return err
	}
	...
	for _, r := range rows {
		...
		if err := s.repo.DeleteRoutine(ctx, r.ID); err != nil {
			return err
		}
	}
	return nil
...
func (s *ConfigService) deleteMissingProjects(
	ctx context.Context, workspaceID string, bundle *ConfigBundle,
) error {
	rows, err := s.repo.ListProjects(ctx, workspaceID)
	if err != nil {
		return err
	}
	...
	for _, p := range rows {
		...
		if err := s.repo.DeleteProject(ctx, p.ID); err != nil {
			return err
		}
	}
	return nil
